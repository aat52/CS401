...
public class Lab7
{
	...
	public static void loadData(MovieDB movies) throws IOException
	{
		...
		int numberOfMovies = S.nextInt();	S.nextLine();

		for (int i = 0; i < numberOfMovies; i++) {
			String title = S.nextLine();
			String director = S.nextLine();
			String studio = S.nextLine();
			double gross = S.nextDouble();	S.nextLine();
	...
	public static void getCommands(MovieDB movies)
	{
		...
		while (true)
		{
			...
			else if (choice.equals("2"))
			{
				...
				inScan.nextLine();	// flush inScan buffer.
			...
			else if (choice.equals("3"))
			{
				...
				String title = inScan.nextLine();
				try {
					System.out.println(movies.findMovie(title).toString());
				} catch (Exception e) {
